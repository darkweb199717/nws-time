  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-8 space-y-8">
      <motion.div className="text-center space-y-2" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <h1 className="text-3xl md:text-4xl font-bold">FD Calculator</h1>
        <p className="text-muted-foreground text-base max-w-xl mx-auto">Calculate your Fixed Deposit maturity amount and interest earned over the investment period.</p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        <motion.div className="xl:col-span-1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
          <Card className="p-6 border-border/50 space-y-6 sticky top-24">
            <h2 className="text-lg font-semibold">FD Details</h2>
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-xs text-muted-foreground"><span>$10K</span><span className="font-medium text-foreground">${parseInt(principal||'0').toLocaleString()}</span><span>$1Cr</span></div>
            </div>
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-xs text-muted-foreground"><span>1%</span><span className="font-medium text-foreground">{rate}%</span><span>15%</span></div>
            </div>
            <div className="space-y-2">
              <Label className="text-sm font-medium flex items-center gap-1"><Calendar className="w-3.5 h-3.5 text-primary" />Tenure (Years)</Label>
              ...
              <div className="flex justify-between text-xs text-muted-foreground"><span>1yr</span><span className="font-medium text-foreground">{years} yrs</span><span>10yrs</span></div>
            </div>
          ...
          {result && (
            <motion.div className="space-y-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  { label: 'Principal', value: `$${result.principal.toLocaleString()}`, bg: 'from-blue-500/10 to-cyan-500/10', border: 'border-blue-500/20', text: 'text-blue-500' },
                  ...
                  { label: 'Maturity Amount', value: `$${result.maturityAmount.toLocaleString()}`, bg: 'from-purple-500/10 to-pink-500/10', border: 'border-purple-500/20', text: 'text-purple-500' },
                ].map((item, index) => (
              ...
              <Card className="p-5 border-border/50">
                <h3 className="text-base font-semibold mb-4">FD Summary</h3>
                <div className="space-y-3">
                  {[
                    { label: 'Principal Amount', value: `$${result.principal.toLocaleString()}` },
                    { label: 'Interest Rate', value: `${result.rate}% p.a.` },
                    { label: 'Tenure', value: `${result.years} years` },
                    { label: 'Interest Earned', value: `$${result.interest.toLocaleString()}` },
                    { label: 'Maturity Amount', value: `$${result.maturityAmount.toLocaleString()}` },
                  ].map((item, i) => (
